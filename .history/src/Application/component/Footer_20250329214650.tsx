...
export function Footer({children}:{children?:ReactNode}){
    ...
    const elementsSources = sources.map((element)=>{
        return <p className="m-5">
            {element.text}<br/>
            <a
                className="color-blue-12 text-decoration-1 m-5"
                href={element.url}
            >
                {element.url}
            </a>
    ...
    return <div   className="position-r w-100 d-bloc ">
        <div style={{height:defaultHeight*1.5}} className={"h-100 d-flex jC-center fd-column "+bgColor} id="PositiveEffect">
            <div className={setClassResponsive("w-100 h-50 p-5 ","","",null,responsiveSize)}>
                <div>
                    Source:
                </div>
                <div>
